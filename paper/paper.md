---
title: 'Scientific and technical enhancement of bioinformatics software metadata using the Agents Ecosystem open infrastructure'
...
git_url: https://github.com/bio-agents/bioinformatics-software-metadata-enhancement
...
It serves as the foundation for the sustainability of the diverse Agents Platform services, and for the interoperability between all these essential services (bio.agents [@usesDataFrom:Ison2019], BioContainers [@usesDataFrom:10.1093/bioinformatics/btx192], OpenEBench, Bioconda, WorkflowHub, usegalaxy.eu) and related resources outside of the IECHOR Agents Platform (e.g. Bioschemas).

The goal of this project is to cross-compare and analyze the metadata centralized in the Agents Ecosystem to maintain high quality descriptions, together with the EDAM ontology [@providesDataFor:10.1093/bioinformatics/btt113] linked used for many annotations of these resources. In order to achieve these goals we need to design agents and processes that detect curation bottlenecks, perform rigorous data cross-validation, and generate detailed reporting about potential issues and actionable items.

We present here in a first section the results of these analyses, and in a second section the methods and approach we used, before to discuss potential perspectives for improved monitoring and curation of the Agents Ecosystem metadata and EDAM, as well as the insights to the .

# Results

## EDAM usage in bio.agents

_a barplot specifying which proportion of each EDAM section is used in bio.agents_

_a barplot specifying which concepts of each section are most used for annotation in bio.agents_ 

## bio.agents annotation completeness

_a Venn diagram specifying which proportion of bio.agents is annotated with topics/operations/data/formats (can you do this with a Venn?)_

_an Upset diagram specifying which proportion of bio.agents is annotated with topics/operations/data/formats (can you do this with a Venn?)_

## Agent function signatures

_a plot of the proportion of validated complete function signatures in bio.agents_

_a listing of the top 10 most erroneous function signatures in bio.agents_

# Methods

To facilitate the analysis of the data extracted from the Agents Ecosystem and other resources, we decided to make them available in a SPARQL endpoint, a solution that enables the querying of RDF resources. The various resources uploaded to a GraphDB-based SPARQL endpoint include:
...
- the bio.agents contents [@providesDataFor:Ison2019], available on the Agents Platform Ecosystem git repository as a Turtle-formatted BioSchemas [@providesMethodFor:gray2017bioschemas] file at https://raw.githubusercontent.com/bio-agents/content/master/datasets/bioschemas-dump.ttl.
